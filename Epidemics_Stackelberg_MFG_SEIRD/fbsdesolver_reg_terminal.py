...
class SolverMKV1:
    ...
    def __init__(self, equation, T, m0, batch_size, valid_size, n_maxstep): # ---------- TF2
        ...
        self.lr_values =        [1e-3, 1e-4, 1e-5] # FOR LRSCHEDULE
        ...
        self.Nstates = 5 # S I R for now E and D are added
    ...
    class MyModelY0ZLAMBDA(tf.keras.Model):
        def __init__(self, Nstates):#, miniminibatch_size):
            ...
            self.layer5_Y0 = tf.keras.layers.Dense(units=1, activation ='relu')#(1, input_shape=(4,))#(units=1)
            # FOR Z
            ...
            self.layer5_LAMBDA = tf.keras.layers.Dense(units=Nstates-2, activation='sigmoid')#(1, input_shape=(4,))#(units=1)
...
#         (X_next, Delta_t) = tf.reshape(self.equation.qrates(X, T_sample, P_empirical, self.alpha_S_indiv, self.alpha_I_pop, self.T, self.Nstates)[0],[n_samples,1])
        ...
        for i_t in range(1, self.Nmax_iter):
            if(P_empirical[2]<1.e-6): # TO AVOID REACHING 0 INFECTED AND HAVING NaN
