...
class SolverMKV1:
    ...
    def __init__(self, equation, T, m0, batch_size, valid_size, n_maxstep): # ---------- TF2        
        ...
        self.basis =            np.eye(self.Nstates)
    ...
    class MyModelY0ZLAMBDA(tf.keras.Model):
        def __init__(self, Nstates):
            ...
            self.layer6_Y0 = tf.keras.layers.Dense(units=1, activation ='relu')
            ...
            self.layer6_Z = tf.keras.layers.Dense(units=Nstates)
            ...
            self.layer6_LAMBDA = tf.keras.layers.Dense(units=Nstates-1, activation='sigmoid')
    ...
    def forward_pass(self, n_samples):
        ...
        for i_t in range(1, self.Nmax_iter):
            if(P_empirical[1]<1.e-6): # capped to avoid reaching 0 infected (resulting in NaNs)
